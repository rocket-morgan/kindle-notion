...
// ============ AUTH ============
const genToken = (u: string) => Buffer.from(`${u}:${getConfig().sessionSecret}`).toString('base64');
const checkToken = (t: string) => { try { const [u,s] = Buffer.from(t, 'base64').toString().split(':'); return u === getConfig().adminUser && s === getConfig().sessionSecret; } catch { return false; } };
...
app.post('/login', async c => {
  ...
  if (u === getConfig().adminUser && p === getConfig().adminPass) {
    setCookie(c, getConfig().cookieName, genToken(u), { httpOnly: true, sameSite: 'Lax', maxAge: 604800, path: '/' });
